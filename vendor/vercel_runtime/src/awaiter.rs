...
//! entirely in-process: there is **no** dedicated wait-until IPC message. With
//! the local lifecycle patch, a request's collector drains before its `end` IPC
//! message. The global collector tracks these drains at shutdown (SIGTERM).
//! In production the drain is unbounded (background
...
/// Collects background futures registered via `waitUntil`. Cheaply cloneable;
/// all clones share the same underlying task set.
...
impl Awaiter {
    ...
    /// Register a background future to be awaited by this collector's drain.
    ///
